...
const Trading = () => {
  ...
  const { wallets, mintAddr, publicKey, baseVault, quoteVault, flag, setFlag } =
    useApp();
  ...
  const init = async () => {
    try {
      ...
      if (baseVault) {
        const base = await connection.getTokenAccountBalance(
  ...
  return (
    <div className="flex flex-col bg-[#121212] pr-[20px] pb-3 pl-[50px] h-[calc(100vh_-_165px)] text-[20px]">
      <div className="pt-2 text-[#E9E1D0]"></div>
        ...
        <div className="text-bright border-secondary col-span-12 p-2 border-r-secondary h-[calc(100vh_-_220px)] min-h-[100px] overflow-y-auto">
          <div className="rounded-md h-full">
            <div className="flex flex-col w-full h-full">
              <div className="flex justify-between items-center pb-2 border-b-2 w-full">
                Trading Panel
              </div>
              <div className="flex flex-col gap-1">
                <div className="">
                  <div className="pb-1">My wallet:{publicKey}</div>
                  <div className="flex gap-3">
                    <div className="inline text-bright">
                    ...
                    <div className="flex items-center">
                      <input
                        type="number"
                        className="bg-bright px-2 rounded-md text-primary"
                        value={sellTokenAmt}
                        onChange={(e) =>
                          setSellTokenAmt(Number(e.target.value))
                        }
                      />
                      <div
                        className="flex justify-center items-center gap-5 bg-sol mx-5 px-2 rounded-md cursor-pointer"
                        onClick={() => handleSellToekns()}
                      >
                        Sell tokens
                        <ClapSpinner loading={mySellLoading} />
                      </div>
                      <div
                        className="flex justify-center items-center gap-5 bg-warning px-2 rounded-md cursor-pointer"
                        onClick={() => handleSetMax()}
                      >
                        Max
                      </div>
                      <div
                        className="flex justify-center items-center gap-5 bg-green mx-5 px-2 rounded-md cursor-pointer"
                        onClick={() => handleRemoveLiquidity()}
                      >
                        Remove All Liquidity
                        <ClapSpinner loading={removeLoading} />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="">
                  Pool Status:
                  <div className="inline text-bright mx-5 px-2">
